...
Glog.prototype.load_plugins = function(options, cb) {
    ...
    for(var i=0;i<options.plugins.length;i++) {
        try {
            ...
            console.log(e);
            try {
                var p = require(options.plugins[i]);
                p.load(GlogPlugin);
            } catch(e) {
              console.log('owell');
            }
        ...
        for(var i=0; i<files.length; i++) {
            ...
            fs.readFile(path.join(article_path, files[i]), 'UTF-8', function(err, data) {
                ...
                articles.push({
                    ...
                    'month'    : month,
                    'body'     : body,
                    'url'      : [year, month, encodeURIComponent(header.url || header.title.replace(/\s/g, '-'))].join('/'),
...
Glog.prototype.req_article = function(req, res, next, options) {
    ...
    var fn = this,
        url = [req.params.year, req.params.month, encodeURIComponent(req.params.article)].join('/');
